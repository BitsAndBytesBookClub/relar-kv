defmodule Kvstore.Partition do
  use Supervisor

  @name __MODULE__

  def start_link(%{partition_id: id} = args) do
    Supervisor.start_link(__MODULE__, args,
      name: String.to_atom("partition_#{args.node_id}_#{id}")
    )
  end
  ...
  def init(args) do
    ...
    id = "#{args.node_id}_#{args.partition_id}"

    children = [
      {Kvstore.LSMPartSupervisor, %{node_id: id}},
      {Kvstore.LSMLevelSupervisor, %{node_id: id}},
      {Kvstore.LSMTreeG, %{node_id: id, lsm_path: lsm_path, compaction_path: lsm_compacted_path}},
      {Kvstore.SSTFileSupervisor, %{node_id: id}},
      {Kvstore.SSTListG, %{node_id: id, path: sst_path}},
      {Kvstore.MemetableG,
       %{
      ...
      {Kvstore.SSTWriterG, %{node_id: id, path: sst_path}}
    ]
